...
Parser.prototype = {
  ...
  parseNodes: function (test) {
      this.collectNodes(() => true, (element) => {
          if (element) this.appendNode(element);
      });
  },

    collectNodes(test, callback) {
        while (this.lexer.hasNext() && test(this.lexer.peekNextToken())) {
            callback(this.parseElement());
        }
    },
  ...
  // ------------------------------------------------------------
    ...
    parseHeader: function () {
        ...
        let content = headerToken.content;
        const tagsMatch = content.match(/^(?<content>.*? )(?<tags>(:[a-z0-9_@%]+)+):$/i);
        let tags = [];
        if (tagsMatch) {
            tags = tagsMatch.groups.tags.split(':').splice(1);
            content = tagsMatch.groups.content;
        }

        const myLevel = headerToken.level;
        let body = [];
        this.collectNodes((token) => {
            return token.type !== Lexer.tokens.header || token.level > myLevel;
        }, element => {
            if (element) {
                body.push(element)
            };
        ...
        body = body.reduce((acc, el) => {
            let last = acc[acc.length - 1];
            if (el.isInline) {
                if (last && last.type === 'paragraph') {
                    last.appendChild(el)
                } else {
                    let par = Node.createParagraph()
                    acc.push(par);
                    par.appendChild(el);
                }
            } else {
        ...
        let taskStatus = null;
        const taskStatusMatch = content.match(/^(?<status>[A-Z]{4,}) /);
        if (taskStatusMatch) {
            taskStatus = taskStatusMatch.groups.status.toLocaleLowerCase();
            content = content.substring(taskStatus.length).trimLeft();
        }

        var header = Node.createHeader([
            this.createTextNode(content) // TODO: Parse inline markups
        ], { level: myLevel, tags, body, taskStatus, });
        this.setNodeOriginFromToken(header, headerToken);

        this.headerMap[content] = header;
        return header;
    },
