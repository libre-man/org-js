...
ConverterHTML.prototype = {
  ...
  convertHeader: function (node, childText, auxData,
    ...
    if (node.taskStatus) {
      childText = this.inlineTag("span", node.taskStatus, {
        "class": "task-status " + node.taskStatus
      }) + this.inlineTag('span', childText);
    }
    ...
    if (node.tags) {
        let tagsContainerInner = node.tags.map(tag => this.inlineTag('span', tag.replace('_', ' '), { class: 'badge badge-secondary' })).join(' ');
        childText += this.inlineTag('div', tagsContainerInner, { class: 'tags-container' });
    }

      if (node.taskStatus) {
          headerAttributes["class"] = "task-status " + node.taskStatus;
      }

      const headerLevel = (this.headerOffset + node.level).toString();
      const header = this.tag(
          "h" + headerLevel,
          childText,
          headerAttributes,
          auxData,
      );
      return header + this.tag(
          'div',
          node.body.map(
              el => this.convertNode(el, recordHeader, insideCodeElement),
          ).join('\n'),
          { class: 'header-body header-body-level-' + headerLevel }
      );
  },
    ...
    if (this.exportOptions.suppressCheckboxHandling) {
      ...
      if (/^\s*\[(X| |-)\]([\s\S]*)/.exec(listItemText)) {
        ...
        var checkboxIndicator = RegExp.$1;
          ...
          var checkboxAttributes = { type: "checkbox", class: 'custom-control-input' };
  ...
  convertLink: function (node, childText, auxData) {
    ...
    if (this.imageExtensionPattern.exec(srcParameterStripped)) {
      ...
      return this.inlineTag("img", null, {
        ...
        let href;
        const headerNode = this.orgDocument.parser.headerMap[node.src.replace(/^\*/, '')];
        if (headerNode) {
            href = '#header-' + (headerNode.sectionNumberText || '').replace(/\./g, '-');
        }
        return this.inlineTag("a", childText, { href });
    } else {
        return this.inlineTag("a", childText, { href: node.src, target: '_blank' });
    }
  },
