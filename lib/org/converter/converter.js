  ...
  computeToc: function (exportTocLevel) {
    ...
    var currentTocs = toc;  // first
      ...
      this.headers = this.headers.sort((a, b) => {
          aNumbers = a.sectionNumberText.split('.');
          bNumbers = b.sectionNumberText.split('.');
          let res = 0;
          for (let i = 0; res === 0 && i < Math.max(aNumbers.length, bNumbers.length); ++i) {
              res = numberCmp(aNumbers[i], bNumbers[i]);
          }
          return res;
      });
